...
- **«دستگاه» = فقط شبکه‌ی IP با prefix /24 (IP limit).** UA در **هویت** دستگاه دخالت ندارد.
  (بازنگری: در ابتدا `hash(UA + IP/24)` بود، ولی چون یک نفر با یک گوشی/یک اینترنت که چند
  کلاینت مختلف را امتحان می‌کند چند UA می‌فرستد و به‌اشتباه چند دستگاه شمرده می‌شد، به IP-only تغییر کرد.)
- **UA همچنان ثبت می‌شود** (هم در ردیف دستگاه به‌عنوان «آخرین کلاینت»، هم کامل در `subscription_logs`
  که در مودال تاریخچه زیر «کلاینت‌ها (User-Agentها)» دیده می‌شود) — فقط روی آن محدودیت گذاشته نمی‌شود.
...
- **همه‌ی افراد پشت یک /24 مشترک (خانه/اداره/NAT) = یک دستگاه** (سهل‌گیری آگاهانه؛ نتیجه‌ی مستقیم IP-only).
- یک نفر که چند کلاینت مختلف را روی یک اینترنت تست می‌کند = یک دستگاه ✅ (هدفِ همین تصمیم).
