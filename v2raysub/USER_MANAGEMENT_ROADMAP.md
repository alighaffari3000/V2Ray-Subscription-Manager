...
> وضعیت کلی: **✅ کامل شد** — همه‌ی فازها (۰ تا ۶) تمام شدند + اصلاحات پساتحویل.
>
> **اصلاحات پساتحویل (روی master):** لینک عمومی کاملاً حذف شد و به کاربر مهاجرت کرد (بدون fallback، نصب تازه بدون لینک پیش‌فرض)؛ مدت `0` و حداکثر دستگاه `0` = نامحدود (پیش‌فرض دستگاه ۳)؛ تاریخچه‌ی استفاده‌ی هر کاربر (UA/IP/زمان + دستگاه‌های متمایز) با endpoint و مودال؛ تنظیمات اسکن خودکار به تب تنظیمات منتقل و دو زیرتب دیگر یکی شدند؛ کارت/API مسیرها و باگ usageChart پاک‌سازی شدند.
>
...
- [x] کلاس `TestUsers` در [test_integration.py](test_integration.py) با ۱۲ تست: auth، ساخت auto-path، path تکراری (جدول users)، تداخل با مسیر سراسری (freeconfigs)، path کوتاه، فعال‌سازی first-use، سرو فعال (کانفیگ واقعی)، dummy منقضی، dummy pause، ۴۰۴ غیرفعال، احترام به output_format=plain، چرخه pause/resume/delete
- [x] تست دستی end-to-end از UI (فاز ۵): ساخت/توقف/ازسرگیری/حذف در مرورگر تأیید شد
- **معیار پذیرش:** ✅ کل suite سبز — **۵۵ تست** (۴۳ قبلی + ۱۲ جدید)، بدون regression.
