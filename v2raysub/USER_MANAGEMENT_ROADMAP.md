...
> وضعیت کلی: **در حال انجام** — فاز جاری: فاز ۵ (UI تب کاربران). فازهای ۰ تا ۴ تمام شدند.
>
> **شاخه‌ی `ui-redesign`:** بازطراحی کامل ظاهر پنل (سایدبار کناری + تم تیره‌ی مدرن + فونت Vazirmatn) روی این شاخه انجام شد. `master` دست‌نخورده (طرح تب‌بالای فاز ۴) برای مقایسه. baseline فازهای ۰–۴ کامیت `94c36ae`.
...
### فاز ۵ — تب کاربران (UI)
- [ ] جدول کاربران: نام، لینک (کپی)، مدت، فعال‌سازی، انقضا، باقیمانده، وضعیت رنگی
- [ ] مودال افزودن/ویرایش (نام، مدت، path سفارشی اختیاری، یادداشت)
- [ ] اکشن‌ها: ویرایش / Pause-Resume / Reset / فعال-غیرفعال / حذف + AJAX
- **معیار پذیرش:** ساخت/ویرایش/حذف/توقف کاربر کاملاً از UI کار می‌کند.
