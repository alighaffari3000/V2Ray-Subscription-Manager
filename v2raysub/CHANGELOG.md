...
### Added
- **صفحه‌ی راهنما وقتی لینک اشتراک در مرورگر باز می‌شود.** تا پیش از این،
  مرورگر یا دیوار base64 کانفیگ‌ها را نشان می‌داد یا (پس از تغییر زیر) پیام
  خنثای `Content not available` را که برای بات‌های پیش‌نمایش نوشته شده بود و
  برای یک آدم واقعی هیچ معنایی نداشت. حالا یک صفحه‌ی فارسی و راست‌به‌چپ نمایش
  داده می‌شود که خود لینک را با دکمه‌ی کپی، مراحل ایمپورت و برنامه‌های پیشنهادی
  (Hiddify به عنوان گزینه‌ی اول) نشان می‌دهد. صفحه هیچ کانفیگی در خود ندارد —
  دقیقاً به همان دلیلی که مسیر بات هم ندارد: وگرنه جعل هدر مرورگر سقف دستگاه را
  دور می‌زد. بدون فونت یا اسکریپت بیرونی (اتصال به CDN همان چیزی است که این
  اشتراک قرار است درستش کند)، سازگار با تم روشن/تیره، و `noindex`.
- وضعیت جدید `BROWSER_VIEW` در لاگ دسترسی‌ها، تا بازدیدهای مرورگر در پنل از
  بات‌های واقعی (`BOT_BLOCKED`) قابل تفکیک باشند.

### Fixed
- **بازدید با مرورگر دیگر یک اسلات دستگاه مصرف نمی‌کند.** کاربر روی لینک
  اشتراکش می‌زد، مرورگر (داخل تلگرام یا Chrome) بازش می‌کرد و همان بازدید به
  عنوان یک دستگاه ثبت می‌شد؛ بعد وقتی همان لینک را در Hiddify ایمپورت می‌کرد و
  IP آن کلاینت در بلاک `/24` دیگری بود (IPv6 در برابر IPv4، یا CGNAT اپراتور)،
  شمارنده در همان اولین استفاده «۲ از ۳» نشان می‌داد. بات‌های پیش‌نمایش از قبل
  مستثنا بودند ولی مرورگرها نه: فهرست `_BOT_UA_MARKERS` مارکر `mozilla` را
  نداشت، درحالی‌که `parse_user_agent` همان User-Agent را `Browser/Bot` (یعنی
  «کلاینت واقعی نیست») طبقه‌بندی می‌کرد. حالا تشخیص مرورگر به همان
  `parse_user_agent` واگذار شده — تنها منبع حقیقتِ طبقه‌بندی UA — که فقط پس از
  ناکام‌ماندن همه‌ی کلاینت‌های شناخته‌شده مقدار `Browser/Bot` برمی‌گرداند، پس
  کلاینتی که UA اش با `Mozilla/5.0` شروع می‌شود اشتباه تشخیص داده نمی‌شود.
  مرورگرها هم مثل بات‌ها کانفیگ واقعی را نمی‌گیرند (به جایش صفحه‌ی راهنمای بالا
  را می‌بینند)، وگرنه جعل هدر مرورگر کل سقف دستگاه را دور می‌زد.

  توجه: دستگاه‌هایی که پیش از این نسخه اشتباهاً ثبت شده‌اند خودبه‌خود پاک
  نمی‌شوند — یا باید از صفحه‌ی دستگاه‌های همان کاربر حذف شوند، یا منتظر
  منقضی‌شدنشان در پنجره‌ی `device_window_days` ماند.
...
- **سخت‌گیری بیشتر در برابر مصرف کامل منابع سرور.** گانیکورن اکنون
  ...
  worker های گیرکرده یا نشتی حافظه‌ی تدریجی بدون نیاز به ری‌استارت دستی
  پاک‌سازی شوند. یک `limit_req` سطح nginx (۱۰ درخواست بر ثانیه هر IP، برست
  ۲۰) قبل از رسیدن درخواست‌ها به دو/سه worker گانیکورن فلاد را مهار می‌کند.
  یک درصدی `SystemMaxUse=200M` هم برای journald تنظیم شد تا لاگ‌های نامحدود
  دیسک را پر نکنند.

### Fixed
- **بیش‌شماریِ ستون «کل کانفیگ» به‌خاطر کانفیگ‌های تکراری.**
  `count_configs_in_text` هر خط کانفیگ را جدا می‌شمرد، پس دو کانفیگی که فقط در
  نامِ نمایشی (بخش بعد از `#`) یا چیدمانِ پارامترها فرق داشتند ولی به همان
  سرور/تنظیمات وصل می‌شدند، دوبار شمرده می‌شدند و عدد بالاتر از چیزی می‌شد که
  کلاینت‌هایی مثل v2rayN نشان می‌دهند (مثلاً 1106 به‌جای ~963 برای یک منبع).
  حالا بر اساس هویتِ فنیِ هر کانفیگ (بدون نام، مستقل از ترتیب پارامترها، با
  نرمال‌سازیِ host/port/credentials/query) dedup می‌شود. تابع کمکیِ محضِ
  `_config_identity_key` اضافه شد. شمارش همچنان بدون تست سلامت و روی همان بدنه‌ی
  fetch‌شده انجام می‌گیرد.

## [1.19.0] - 2026-07-25

بازبینی نهایی API ماشینی پیش از ساخت کلاینت خارجی. همه‌ی موارد زیر با اجرای کد
بازتولید و سپس با تست رگرسیون پوشش داده شدند.

### Fixed
- **تمدیدهای همزمان گم می‌شدند (اثر مالی).** `extend_user` انقضا را در پایتون
  می‌خواند و بعد می‌نوشت؛ دو تمدید همزمان (مثلاً ری‌ترای وبهوک درگاه) هر دو یک
  مقدار می‌خواندند و یکی بازنویسی می‌شد: مشتری دو بار پرداخت می‌کرد و یک تمدید
  می‌گرفت، و `duration_days` هم با انقضا ناسازگار می‌شد. اکنون کل محاسبه در یک
  `UPDATE` اتمیک انجام می‌شود.
- **تمدید بیش از حد، اشتراک را همیشه‌رایگان می‌کرد.** برای `days`ی که تاریخ را از
  سال ۹۹۹۹ عبور می‌داد، `datetime()` در SQLite مقدار NULL برمی‌گرداند؛ نتیجه یک
  اشتراکِ فعالِ بدون انقضا بود که هیچ‌وقت منقضی نمی‌شد و در پنل هم «فعال نشده»
  نمایش داده می‌شد. سقف `USER_MAX_DURATION_DAYS` اضافه شد.
- **تمدید اشتراک متوقف (paused)، باقی‌مانده‌ی منجمد را نابود می‌کرد** و سپس
  `resume` کل مدت توقف را هم اضافه می‌کرد: اشتراکی که ۱۰۰ روز متوقف بود و ۵ روز
  اعتبار داشت، با تمدید ۳۰ روزه به **۱۳۰ روز** می‌رسید. اکنون برای اشتراک متوقف
  به‌جای مبنا گرفتن «الان»، به همان انقضای منجمد اضافه می‌شود (نتیجه: ۳۵ روز).
- **`max_devices` منفی پذیرفته می‌شد و سقف دستگاه را برمی‌داشت** (چون `0` یعنی
  نامحدود)؛ یعنی دور زدن همان چیزی که پلن را قیمت‌گذاری می‌کند. حالا رد می‌شود.
- **یک فیلد نامعتبر، بقیه‌ی فیلدهای همان درخواست را بی‌صدا حذف می‌کرد.** در
  `update_user` پلیس‌هولدر SQL قبل از `int()` اضافه می‌شد، پس با ورودی نامعتبر یک
  `?` بدون مقدار می‌ماند و کل دستور می‌مرد (این پنل ادمین را هم درگیر می‌کرد).
- **توکن غیر-ASCII باعث خطای ۵۰۰ در مسیر پیش‌از‌احراز می‌شد** (`compare_digest`
  روی رشته‌ی غیر-ASCII استثنا می‌دهد) — روی مسیری که از rate-limit معاف است.
  مقایسه اکنون بایتی است و ۴۰۱ برمی‌گرداند.
- **خطاهای داخلی دیتابیس به‌صورت ۴۰۴ گزارش می‌شدند** در اندپوینت‌های دستگاه، که
  کلاینت را به حذف رکوردِ اشتراکِ موجود ترغیب می‌کرد. اکنون ۴۰۴ فقط برای نبودِ
  واقعی، و ۵۰۰ برای خطای داخلی.
- **اعتبارسنجی «آدرس عمومی پنل» فقط پیشوند را چک می‌کرد**، پس مقداری مثل
  `https://` پذیرفته می‌شد و لینک‌ها را به `https://sub/...` تبدیل می‌کرد — و چون
  همین مقدار لینک‌های پنل ادمین را هم می‌سازد، آن‌ها را نیز خراب می‌کرد. اکنون با
  `urlsplit` یک origin واقعی الزامی است (مسیر/پارامتر رد می‌شود، پروتکل با حروف
  بزرگ پذیرفته می‌شود).
- **کرش مهاجرت چندورکری (نهفته).** در `_add_column_if_missing` دستور `ALTER`
  کنترل‌نشده بود؛ چند ورکر gunicorn هم‌زمان می‌توانستند هر دو ستون را اضافه کنند و
  دومی بوت ورکر را می‌شکست. اولین نسخه‌ای که ستون اضافه می‌کرد این را فعال می‌کرد.

### Changed
- **پاسخ خطاها ماشین‌خوان شد.** کدهای `invalid_request` (به‌همراه `field`)،
  `path_taken`، `device_not_found` و `internal_error` اضافه شدند تا کلاینت مجبور
  نباشد متن فارسی پیام را تفسیر کند. **ناسازگاری با ۱.۱۸.۰:** مسیر تکراری اکنون
  `409 path_taken` می‌دهد (پیش‌تر `400 create_failed`).
- **ساخت اشتراک ایدمپوتنت شد.** اگر کلاینت `path` قطعی خودش را بدهد، تکراری بودن
  با `409` و **همراهِ اشتراکِ موجود** برمی‌گردد؛ پس ری‌ترای وبهوک، اشتراک اول را
  بازمی‌یابد و اشتراک دوم ساخته نمی‌شود.
- **تایپ‌ها سخت‌گیرانه شدند.** فیلدهای عددی، رشته/اعشار/بولین را رد می‌کنند و
  `enabled` باید بولین واقعی باشد — پیش‌تر `{"enabled": "false"}` به‌خاطر truthiness
  اشتراک غیرفعال را **فعال** می‌کرد.
- **آدرس پایه یک‌بار در هر درخواست حل می‌شود.** پیش‌تر برای هر سطر یک کانکشن تازه‌ی
  SQLite باز می‌شد؛ فهرست ۲۰ اشتراکی ۲۳ کانکشن می‌گرفت (اکنون ۴)، و همین رگرسیون
  روی `/adminpanel/api/users` هم برطرف شد (۲۲ → ۳).

### Security
- **مجوز دیتابیس از `644` به `640` تغییر کرد** (به‌همراه فایل‌های `-wal`/`-shm`).
  از ۱.۱۷.۰ توکن API داخل دیتابیس است و جهان‌خوان بودنِ آن یعنی هر کاربر محلیِ
  بی‌دسترسی می‌توانست اعتبارنامه‌ی کنترل کاملِ اشتراک‌ها را بخواند.

### Added
- **نصاب روی پورت غیراستاندارد، `public_base_url` را خودش پین می‌کند.** nginx هدر
  `Host $host` را می‌فرستد و `$host` پورت را حذف می‌کند، پس نصب روی مثلاً `:8443`
  باعث می‌شد هر لینکِ ساخته‌شده‌ی پنل بدون پورت و غیرقابل‌دسترس باشد. روی ۸۰/۴۴۳
  خالی می‌ماند (تا لینک‌ها دامنه را خودکار دنبال کنند) و مقدار دستی ادمین هرگز
  بازنویسی نمی‌شود.
- **نصاب در پایان، آدرس API ماشینی را نمایش می‌دهد** و یادآوری می‌کند که تا تولید
  توکن غیرفعال است.
- مستندسازی کدهای خطا و رفتار ایدمپوتنسی در README.
...
### Added
- **ستون «منبع» در جدول «تاریخچه آخرین اسکن‌ها».** نشان می‌دهد هر اسکن روی
  چه چیزی اجرا شده: «همه منابع» برای اسکن کامل خودکار/زمان‌بندی‌شده، نام همان
  منبع برای اسکنِ تک‌منبعیِ دکمه‌ی 🧪 (تست دستی)، و «—» برای اسکن نوع
  «بررسی سلامت» (چون آن نوع مستقل از منبع، همه‌ی کانفیگ‌های فعال را چک می‌کند).
  ستون جدید `scan_history.source_scope` (مهاجرت idempotent) همان لحظه‌ی شروع
  اسکن در `run_scan` پر می‌شود.
